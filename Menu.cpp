...
void Menu::validPlayerNames(istream& inStream, string* p1Name,
                            string* p2Name, bool* valid){
    string line = "";
...
void Menu::validNextPlayer(istream& inStream, string p1Name,
                           string p2Name, bool* valid) {
    string nextPlayer = "";
    if(*valid == true){
        inStream >> nextPlayer;
...
void Menu::validFactoryChar(char c, bool* valid, int i) {
    if(c != BLUE && c != YELLOW && c != RED && c != BLACK &&
       c != LIGHT_BLUE && c != EMPTY) {
        *valid = false;
        cout << "Error - Invalid Factory Character: " << c << " in Factory " 
             << i+1 << endl;
    }
}
...
void Menu::validFactoryLength(string line, bool* valid, int i) {
    ...
    if(line.size() != MAX_CHARS_FOR_FACTORY 
       && (line.size() != (MAX_CHARS_FOR_FACTORY + 1))) {
        *valid = false;
        cout << "Error - Invalid Factory Length: " << line.size() 
             << " in Factory " << i+1 << endl;
    }
}
...
void Menu::validFacZandBrokenChar(char c, bool* valid){
    if(c != BLUE && c != YELLOW && c != RED && c != BLACK && c != LIGHT_BLUE 
       && c != FIRST_PLAYER_MARKER) {
        *valid = false;
        cout << "Error - Invalid Factory Zero or Broken Storage Character: " 
             << c << endl;
    }
}
        ...
        for(int i = 0; i < NUM_OF_PLAYERS; ++i){
            for(int rowNum = 0; rowNum < MAX_MOSAIC_ROW_NUM; ++rowNum){
                ...
                if(line.size() != MAX_CHARS_FOR_MOSAIC 
                   && (line.size() != (MAX_CHARS_FOR_MOSAIC+1))) {
                    *valid = false;
                    cout << "Error - Invalid Mosaic Size: " << line.size() 
                         << " in Mosaic Row " << rowNum+1 << endl;
                } else if(*valid) {
...
void Menu::validMosaicChar(string row, bool* valid, int rowNum){
    ...
    for(int i = 0; i != MAX_MOSAIC_ROW_NUM; ++i){
        if(*valid) {
            if(!(row[i] == tiles[start%5] 
               || row [i] == tolower(tiles[start%5]))) {
                 *valid = false;
                 cout << "Error - Invalid Mosaic Character: " 
                      << row[i] << " in Mosaic Row " << rowNum+1 << " Column " 
                      << i + 1 << endl;
            }
            ++start;
        }    
...
void Menu::validMosaicRow(string row, bool* valid, int rowNum) {
    ...
    for(int i = 0; i != MAX_CHARS_FOR_MOSAIC; ++i){
        if(i%2 == 1){
            if((row[i] != ' ') && (*valid)) {
                *valid = false;
                cout << "Error - Invalid Mosaic Character: " << row[i] 
                     << " in Mosaic Row " << rowNum+1 << endl;
            }
        } else {
...
void Menu::validStorageSize(istream& inStream, bool* valid, int index,
                            string line) {
    if(*valid == true){
        if(index == 0){
            if(line.size() != 1 && line.size() != 2){
                *valid = false;
                cout << "Error - Invalid Storage Size: "<< line.size() 
                     << " in Storage Row " << index + 1 << endl;
            }
    } else if(index > 0){
            int size = line.size();
...
void Menu::validStorageChar(char c, bool* valid, int i){
     if(c != BLUE && c != YELLOW && c != RED && c != BLACK && c != LIGHT_BLUE 
        && c != EMPTY) {
        *valid = false;
        cout << "Error - Invalid Storage Character: "<< c << " in Storage Row " 
             << i << endl;
    }
}
    ...
    if(*valid == true){
        for(int i = 0; i < NUM_OF_PLAYERS; ++i){
            ...
            if(line.size() > MAX_CHARS_FOR_BROKEN){
                *valid = false;
                cout << "Error - Invalid Broken Storage Size: " << line.size() 
                     << endl;
...
void Menu::validTiles(string filename, bool* valid){
    ...
    if(*valid){
        if(!(redTiles == 20 && blueTiles == 20 && blackTiles == 20 
           && yellowTiles == 20 && lightBlueTiles == 20)) {
            *valid = false;
            cout << "Error - Invalid number of tiles: " << "R = " << redTiles 
                 << ", B = " << blueTiles << ", U = " << blackTiles << ", Y = " 
                 << yellowTiles << ", L = " << lightBlueTiles << endl;
        }
    }
}